...
import settings
from main import get_result, round_number_to_decimal
...
class TestInitial:
    ...
    def test_closest_exchange_rate_has_empty_rate_then_work_with_the_next_closest_not_empty_rate(self, db_migrations, db_data):
        db_conn = db_migrations(settings.MIGRATION_INITIAL)
        with closing(db_conn) as conn:
            db_data(conn, 'exchange_rates', [
    ...
    def test_exchange_rate_to_gbp_without_timestamp(self, db_migrations, db_data):
        db_conn = db_migrations(settings.MIGRATION_INITIAL)
        with closing(db_conn) as conn:
            db_data(conn, 'exchange_rates', [
                (None, 'USD', 'GBP', '0.71'),
    ...
    def test_exchange_rate_to_gbp_without_from_currency(self, db_migrations, db_data):
        db_conn = db_migrations(settings.MIGRATION_INITIAL)
        with closing(db_conn) as conn:
            db_data(conn, 'exchange_rates', [
                ('2018-04-01 00:00:00', None, 'GBP', '1.5'),
            ])
            db_data(conn, 'transactions', [
                ('2018-04-01 00:30:00', 2, 'USD', 2.45),
            ])
